...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const qLogs = query(collection(db, 'attendance_logs'), orderBy('date', 'desc'));
    const unsubLogs = onSnapshot(qLogs, (snap) => {
      setAttendanceLogs(snap.docs.map(d => ({ id: d.id, ...d.data() })));
      setLoading(false);
    });

    return () => { unsubTerms(); unsubLogs(); };
  }, []);
  ...
  const validateTerm = () => {
    ...
    if (newTerm.startDate && newTerm.endDate && newTerm.endDate < newTerm.startDate)
      e.endDate = true;
    setTermErrors(e);
  ...
  const filteredLogs = attendanceLogs.filter(log => {
    ...
    if (filterClass && !log.classId?.toLowerCase().includes(filterClass.toLowerCase())) return false;
    if (filterHouse && !log.house?.toLowerCase().includes(filterHouse.toLowerCase()))   return false;
    return true;
  });
      ...
      <Box sx={{ mb: 4 }}>
        ...
        {terms.length === 0 ? (
          <Alert severity="warning">
            No terms created yet. Add a term to start recording attendance.
          </Alert>
        ) : (
          <Stack
            ...
            {terms.map(term => (
              <Card
                ...
                sx={{
                  minWidth: 260,
                  flex: '1 1 260px',
                  borderColor: term.isActive ? 'primary.main' : 'divider',
                  borderWidth: term.isActive ? 2 : 1,
                  borderRadius: 2,
                }}
              >
                <CardContent>
                  <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 1 }}>
                    <Typography
                      variant="h6"
                      color={term.isActive ? 'primary.main' : 'text.primary'}
                      fontWeight="bold"
                    >
                      {term.name}
                    </Typography>
      ...
      <Card sx={{ mb: 4 }}>
        <CardContent>
          <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
            Records Browser
            {activeTerm
              ? ` — ${activeTerm.name} (${activeTerm.startDate} to ${activeTerm.endDate})`
              : ' — No active term'}
          </Typography>
          ...
          <Box sx={{
            ...
            <TextField
              ...
              value={filterHouse}
              onChange={(e) => { setFilterHouse(e.target.value); setPage(0); }}
            />
            {(filterDate || filterClass || filterHouse) && (
              <Button
                variant="text"
                onClick={() => { setFilterDate(''); setFilterClass(''); setFilterHouse(''); setPage(0); }}
                sx={{ height: '56px' }}
              >
                Clear Filters
              </Button>
                ...
                .map((log) => (
                  <Card key={log.id}>
                    <CardContent>
                      ...
                      <Divider sx={{ my: 1 }} />
                      <Typography variant="body2" fontWeight="bold">
                        Present: {log.totalPresent} &nbsp;|&nbsp;
                        Absent: {log.totalAbsent} &nbsp;|&nbsp;
                        Total: {log.totalStudents}
                      </Typography>
                    </CardContent>
                  </Card>
                ))}
              ...
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ bgcolor: 'primary.main' }}>
                    {[
                      'Date', 'Class', 'House', 'Submitted By',
                      'GB (P/A)', 'GD (P/A)', 'BB (P/A)', 'BD (P/A)',
                      'Total P', 'Total A', 'Total',
                    ].map((h) => (
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            <TextField
              ...
              helperText={termErrors.name ? 'Term name is required' : ''}
            />
            <TextField
              label="Start Date"
              type="date"
              ...
              helperText={termErrors.startDate ? 'Start date is required' : ''}
              slotProps={{ inputLabel: { shrink: true } }}
            />
            <TextField
              label="End Date"
              ...
              helperText={termErrors.endDate ? 'End date required and must be after start date' : ''}
              slotProps={{ inputLabel: { shrink: true } }}
            />
          </Stack>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => { setOpenAddTerm(false); setTermErrors({}); setNewTerm(EMPTY_TERM); }}>
            Cancel
          </Button>
